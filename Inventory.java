...
class Inventory {
        ...
        for (int i = 0; i < SIZE; i ++) 
        {
            if(inventory[i] != null && inventory[i].inStock())
            {
                double price = inventory[i].getPrice();
                    System.out.println(inventory[i].getName() + " costs " + decFor.format(price/100) + 
                            " and there are " + inventory[i].getQuantity() + " of them.");
            }
        }
    }
    ...
    protected boolean inStock(int loc)
    {
        if(inventory[loc]!=null)
        {
            return inventory[loc].inStock();
        }
        return false;
    }
